...
class UserBalanceModel {
  ...
  UserBalanceModel({required this.availableBalance, this.withdrawAmount = '0'});
}

class UserBalanceViewModel extends ChangeNotifier {
  ...
  void updateWithdrawAmount(String digit) {
    if (_userBalanceModel.withdrawAmount == '0') {
      ...
      if (int.parse(_userBalanceModel.withdrawAmount + digit) <
          availableBalance) {
        _userBalanceModel.withdrawAmount += digit;
      }
    }
  ...
  void confirmWithdraw() {
    ...
    if (amount <= _userBalanceModel.availableBalance) {
